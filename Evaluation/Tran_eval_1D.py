...
class Evaluation:
    ...
    def sample_evaluation(self, xA_test, xB_test, gen_A_B, epoch, obj=''):
        ...
        function_choice = np.random.randint(0, xA_test.shape[0])
        # notice the testset is paired, which allows for better comparison
        f_A = xA_test[function_choice]
        f_B = xB_test[function_choice]
        # convert the image into tensors whose shape can be fed into the network
        choice_A = f_A[np.newaxis, :, :]
        choice_B = f_B[np.newaxis, :, :]
        ...
        plt.title('Input')
        #plt.axis('off')
        plt.plot(self.time_axis, choice_A[0, :, 0], color='blue')
        ...
        #plt.axis('off')
        plt.plot(self.time_axis, trafo[0, :, 0], color='cyan')

        plt.subplot(133)
        plt.title('Target and Gen out')
        #plt.axis('off')
        plt.plot(self.time_axis, trafo[0, :, 0], color='green')
        plt.plot(self.time_axis, choice_B[0, :, 0], color='cyan')

        plt.subplot(144)
        plt.title('Error')
        plt.plot(self.time_axis, (np.abs(choice_B - trafo))[0, : , 0], color='red')


        eval_loss = np.mean(np.abs(choice_B - trafo))
        print("Eval_loss: ", eval_loss)

        plt.suptitle('MAE '+ obj + ' Loss: ' + str(eval_loss)[0:7] + ' on image ' + str(function_choice))
    ...
    def sample_best_model_output(self, xA_test, xB_test, gen_A_B, gen_name, obj='', amount=10):
        ...
        for i in range(amount):
            # notice the testset is paired, which allows for better comparison
            f_A = xA_test[function_choice[i]]
            f_B = xB_test[function_choice[i]]
            # convert the image into tensors whose shape can be fed into the network
            choice_A = f_A[np.newaxis, :, :]
            choice_B = f_B[np.newaxis, :, :]
...
class GAN_evaluation(Evaluation):
    ...
    def evaluate_gan_on_testdata_chunk(self, generator, gen_name, discriminator, patch_data, xA_test, xB_test, epoch, obj='', full_test_data=True, chunk_split=0.3, shuffle=True):
        if full_test_data:
            xA = xA_test
            xB = xB_test
        else:
