...
class Evaluation:
    def __init__(self, Modelname, Type, data_config, evaluation_loss_mode=0, use_eval_results_dir=True):
        # - file path info -
        self.Model_name = Modelname
        self.test_path = Modelname + '_Evaluation1D'
        if use_eval_results_dir:
            self.test_path = '../../Evaluation Results/' + Type + '/' + self.test_path
...
class VAE_evaluation(Evaluation):
    ...
    def __init__(self, modelname, cfg, evaluation_loss_mode=0):
        super().__init__(modelname, 'VAE', cfg, evaluation_loss_mode)
    ...
    def q_save_model_NO(self, validation_loss_no_epoch, epoch):
        if self.best_loss_no > validation_loss_no_epoch[0]:      # 0 is imageloss, 1 featureloss
            self.best_loss_no = validation_loss_no_epoch[0]
...
class Generator_evaluation(Evaluation):
    ...
    def __init__(self, modelname, cfg, evaluation_loss_mode=0):
        super().__init__(modelname, 'Gen', cfg, evaluation_loss_mode)
...
class GAN_evaluation(Evaluation):
    def __init__(self, modelname, cfg,evaluation_loss_mode=0):
        super().__init__(modelname, 'GAN', cfg,evaluation_loss_mode)
        self.training_history_dir   = self.test_path + '/' + self.Model_name + ' Training History'
        self.model_saves_dir        = self.test_path + '/' + self.Model_name + ' Trained Models'
        os.makedirs(self.training_history_dir, exist_ok=True)
        os.makedirs(self.model_saves_dir, exist_ok=True)
    ...
    def dump_training_history_GAN(self):
        ...
        training_history_GAN_GES['loss']        = [g[0] for g in self.gan_ges_loss_epoch]
        training_history_GAN_GES['std']         = [g[1] for g in self.gan_ges_loss_epoch]
        training_history_GAN_GES['val_loss']    = self.gan_valid_loss_no

        with open(self.training_history_dir + '/GES_LOSS_GAN', "wb") as fp:
            pickle.dump(training_history_GAN_GES, fp)
