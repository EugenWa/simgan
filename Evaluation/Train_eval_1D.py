...
class Evaluation:
    def __init__(self, Evaluation_Result_Location , Modelname, Type, Model_config_path, data_config, incl_features=False,evaluation_loss_mode=0, use_eval_results_dir=True):
        # - file path info -
    ...
    def calculate_loss_outside_errorbars(self, Target, Prediction):
        ...
        for channel in range(Target.shape[1]):
            ...
            intollerable_error = np.sum(error) / (error.shape[0] - tollorable_error_len) # this is not used
            general_error += intollerable_error
...
class VAE_evaluation(Evaluation):
    ...
    def __init__(self, Evaluation_Result_Location, modelname, Model_config_path, cfg, incl_features=False, evaluation_loss_mode=0):
        super().__init__(Evaluation_Result_Location, modelname, 'VAE',  Model_config_path, cfg, incl_features, evaluation_loss_mode)
...
class GAN_evaluation(Evaluation):
    def __init__(self, Evaluation_Result_Location,  modelname,  Model_config_path, cfg, incl_features=False, evaluation_loss_mode=0):
        super().__init__(Evaluation_Result_Location, modelname, 'GAN', Model_config_path, cfg, incl_features, evaluation_loss_mode)
    ...
    def evaluate_gan_minimalistic(self, gen_model, disc_model, xA_test, xB_test, patch_data):
        ...
        for p_i in range(patch_data[0]):
            disc_eval_indiv_patch_score.append(disc_model.evaluate(disc_eval_batch[:, p_i * patch_data[1]:(p_i + 1) * patch_data[1], :], disc_eval_labels))
