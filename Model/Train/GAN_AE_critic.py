...
from Model.Modules.GANs.Basic_AE_GAN_critic             import Basic_AE_Gan_Critic
...
if __name__ == "__main__":
    ...
    # generator
    config_name_gan      = 'GAN_Sqrs_ID_01.ini'#sys.argv[1]  #'gan_cfg.ini'#sys.argv[1] #'gan_cfg_IDo_1.ini'
    config_name_ae       = 'GAN_nRES_AE_Sqrs_ID_01_pretrain.ini'#sys.argv[2]
    degraded_dataset     = 'B1'#sys.argv[3]
    ...
    gan_eval    = GAN_evaluation('../../Evaluation Results/', gan_config['MODEL_NAME'], '../Configs' + '/' + config_name_gan, Data_loader.config, error_mode)
    ae_eval     = VAE_evaluation('../../Evaluation Results/', ae_config['MODEL_NAME'],  '../Configs' + '/' + config_name_ae,  Data_loader.config, error_mode)
    ...
    # --- Build-Model ---
    GAN = Basic_AE_Gan_Critic(gan_config['MODEL_NAME'], xA_train[0].shape, gan_config, ae_config, gan_eval, ae_eval)
    ...
    for epoch in range(epochs_id_gan + epochs_norm_gan):
        ...
        for batch_i in range(number_of_iterations):
            ...
            for _ in range(GAN_DISC_RATIO):
                # --- train discriminator ---
                disc_batches = [xA_batch, imgs_vae]
                disc_labelss = [np.ones(xA_batch.shape[0], ), np.zeros(imgs_vae.shape[0], )]
                disc_loss = []
                for q in range(1):
                    disc_batch, disc_labels = create_disc_batch(xA_batch, imgs_vae)#disc_batches[q], disc_labelss[q]#
                    ...
                    if disc_train_mode == 'MIN':
                        ...
                        for p_i in p_i_S:
                            ...
                            GAN.clip_weights()
                    elif disc_train_mode == 'MAX':
                        p_i_S = range(GAN.patch_length_width)
                        for p_i in p_i_S:
                            disc_train_loss_max = GAN.discriminator.train_on_batch(disc_batch[:, p_i * GAN.patch_width:(p_i + 1) * GAN.patch_width, :], disc_labels)
                            disc_loss.append(disc_train_loss_max)
                            GAN.clip_weights()
