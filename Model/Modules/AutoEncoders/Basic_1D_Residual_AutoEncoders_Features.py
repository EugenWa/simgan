...
from Model.Modules.Generators1D.Generator_Basic_Residual import build_basic_decoder_residual, build_basic_encoder_residual, build_basic_transformation_layers
from Model.Modules.Generators1D.Feature_Classifier import build_feature_classifier
from Model.Modules.Static_Objects import possible_loss_functions, possible_optimizers


class Basic_nRES_ResidualAE:
    def __init__(self, ae_name, img_shape, cfg, AE_eval):
        ...
        inp = Input(img_shape)
        feat, skippings = build_basic_encoder_residual(inp, filters, SKIPS, relu_param, use_batch_normalisation, use_drop_out)
        if trafo_layers > 0:
            tfeat = build_basic_transformation_layers(feat, trafo_layers, filters[-1], relu_param, use_batch_normalisation, use_drop_out)
        else:
            tfeat = feat
        out = build_basic_decoder_residual(tfeat, list(reversed(skippings)), True, list(reversed(filters)), SKIPS, relu_param, use_batch_normalisation, use_drop_out)
        ...
        if classify_mode is 1:
            out_classification = build_feature_classifier(tfeat, CLASSIF_FILTERS, CLASSIF_DENSES, relu_param, use_batch_normalisation, use_drop_out)
            self.auto_encoder = Model(inp, outputs=[out, out_classification], name=ae_name)  # classif, and feat are equally important
            self.auto_encoder.compile(optimizer=ae_optimizer, loss=[autoencoder_loss, CLASSIF_LOSS])
        elif classify_mode is 2:
            out_classification = build_feature_classifier(tfeat, CLASSIF_FILTERS, CLASSIF_DENSES, relu_param, use_batch_normalisation, use_drop_out)
            self.auto_encoder = Model(inp, outputs=[out_classification], name=ae_name)
            self.auto_encoder.compile(optimizer=ae_optimizer, loss=CLASSIF_LOSS)

        else:
            self.auto_encoder = Model(inp, out, name=ae_name)
            self.auto_encoder.compile(optimizer=ae_optimizer, loss=autoencoder_loss)

        print('Basic nRes Residual AE: ')
