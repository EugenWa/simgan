# - keras -
import keras
from keras.layers import BatchNormalization, Input, Conv2DTranspose, LeakyReLU, Add, Lambda, MaxPooling2D, \
...
def build_basic_encoder(encoder_input, e_filters=[3, 9, 21, 33], relu_param=0.3, use_batch_normalisation=True, use_dropout=False):
    ...
    for i in range(len(e_filters)):
        c_L = Conv2D(e_filters[i], kernel_size=filter_len, strides=(1, 1), padding='same')(c_L)
...
def build_basic_decoder(decoder_input, d_filters=[33, 21, 9, 3], relu_param=0.3, use_batch_normalisation=True, use_dropout=False):
    ...
    for i in range(len(d_filters)):
        c_L = Conv2D(d_filters[i], kernel_size=filter_len, strides=(1, 1), padding='same')(c_L)
        c_L = LeakyReLU(relu_param)(c_L)
    ...
    dec_output = Conv2D(1, kernel_size=(3, 3), strides=(1, 1), padding='same')(c_L)     # add activatoin sigmoid
