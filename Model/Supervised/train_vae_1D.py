...
if __name__ == "__main__":
    ...
    ####################
    vae = Basic_2Channel_VAE(vae_name, xA_train[0].shape, vae_config)
    ####################
    ...
    for epoch in range(epochs_id + epochs_norm):
        ...
        validation_loss_id_epoch = vae_eval.evaluate_model_on_testdata_chunk_gen_only(vae.predict_ID_img_only, vae.VAE_ID.name, xa_val, xa_val, epoch, 'ID') #vae.VAE_ID.evaluate(xa_val, xa_val, batch_size, verbose=1)
        vae_eval.valid_loss_id.append(validation_loss_id_epoch)
        #vae_eval.evaluate_model_on_testdata_chunk_gen_only(vae.predict_ID_img_only, vae.VAE_ID.name, xa_val, xa_val,
          #                                                 epoch, 'ID')
        vae_eval.visualize_chunk_data_test(vae.VAE_ID.name, xa_val, xa_val, epoch, 'ID')
        # save best id model
        ...
        if epoch >= epochs_id:
            #_, feature_pred = vae.VAE_ID_MultiOut.predict(xa_val)
            validation_loss_no_epoch = vae_eval.evaluate_model_on_testdata_chunk_gen_only(vae.predict_NO_img_only, vae.VAE_NO.name, xb_val, xa_val, epoch, 'NO')#vae.VAE_NO.evaluate(xb_val, [xa_val, feature_pred], batch_size, verbose=1)
            vae_eval.valid_loss_no.append(validation_loss_no_epoch)
            vae_eval.visualize_chunk_data_test(vae.VAE_NO.name, xb_val, xa_val, epoch, 'NO')
    ...
    vae_eval.visualize_best_samples(vae.VAE_ID.name, 'BEST_ID_id')
    vae_eval.visualize_best_samples(vae.VAE_NO.name, 'BEST_ID_no')
    ...
    vae_eval.visualize_best_samples(vae.VAE_ID.name, 'BEST_NO_id')
    vae_eval.visualize_best_samples(vae.VAE_NO.name, 'BEST_NO_no')
