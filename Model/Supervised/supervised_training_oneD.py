...
if __name__ == "__main__":
    ...
    history_id = vae_res.fit(xA_train, xA_train, validation_data=(xa_val, xa_val), shuffle=True, epochs=epochs_id, batch_size=b_size_id, callbacks=[checkpointer0])
    ...
    history_no = vae_res.fit(xB_train, xA_train, validation_data=(xb_val, xa_val), shuffle=True, epochs=epochs_norm, batch_size=b_size_no, callbacks=[checkpointer])
    ...
    # safe history
    with open(vae_eval.training_history_dir + '/Hist_ID', "wb") as fp:
        pickle.dump(history_id.history, fp)
    with open(vae_eval.training_history_dir + '/Hist_NO', "wb") as fp:
        pickle.dump(history_no.history, fp)
